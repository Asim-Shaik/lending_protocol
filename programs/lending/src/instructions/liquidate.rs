...
use crate::{constants::{MAXIMUM_AGE, SOL_USD_FEED_ID, USDC_USD_FEED_ID}, error::ErrorCode};
...
pub fn process_liquidate(ctx: Context<Liquidate>)-> Result<()> {
    ...
    match ctx.accounts.collateral_mint.to_account_info().key() {
        key if key == user.usdc_address => {
          ...
          let new_sol= calculate_accrued_interest(user.borrowed_sol,borrowed_bank.interest_rate, user.last_updated_borrowed)?;
        ...
        _=>{
          ...
          let new_usdc= calculate_accrued_interest(user.borrowed_usdc,borrowed_bank.interest_rate, user.last_updated_borrowed)?;
